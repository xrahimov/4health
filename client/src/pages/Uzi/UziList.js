...
const UziList = ({ auth: { token } }) => {
    ...
    axios
      ...
      .then((response) => {
        if (response.status === 200) {
          ...
          console.log('Failed to fetch patient datas');
        }
      });
  };
  ...
  socket.on('new-uziappointment', (data) => {
            ...
            {appointments[0] &&
              appointments.map((appointment) => (
                <Patient key={appointment._id}>
                  ...
                  <td>+998{appointment.patient.phonenumber}</td>
                  <td>{dateFormatter(appointment.patient.birthday)}</td>
                  <td>
                    <Link to={`/uzi/${appointment._id}`}>Тўлдириш</Link>
                  </td>
                </Patient>
              ))}
          </tbody>
...
export default compose(requireLabarant, connect(mapStateToProps))(UziList);
